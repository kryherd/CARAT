...
data1 = "./SPR1_sub%s.txt" % (args.subj)
data2 = "./SPR2_sub%s.txt" % (args.subj)

def create_timing(data):
	df=pd.read_table(data)
	# remove all practice trials
	df_real = df[df['imagetype'].str.match("SPR")].copy()
...
for ttype in run1['type'].unique():
	emo1 = run1[run1['type'] == ttype]
	emo2 = run2[run2['type'] == ttype]
